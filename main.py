...
RAW_DB_ID = os.environ.get("DATABASE_ID", "1ece01bb658c4f2aa38398d771127eed").strip()
DATABASE_ID = RAW_DB_ID.replace("[", "").replace("]", "").replace("/", "").strip()
...
# Bot & Gemini AI Initialization
bot = telebot.TeleBot(TELEGRAM_TOKEN)
genai.configure(api_key=GEMINI_KEY)

# Flask Server for Render 24/7 Port Keep-Alive
app = Flask(__name__)

@app.route('/')
def home():
    return "🚀 CA Tracker Pro (AIR-1 UPSC Standard) 24/7 Engine is Live & Running!"

# Global Session Memory for 2-Stage Interactive Selection
USER_SESSIONS = {}

# ==============================================================================
# 2. NOTION SCHEMA FETCH & RICH-TEXT PARSERS
# ==============================================================================
def get_database_properties():
    """Notion database ke active schema columns dynamically fetch karta hai"""
    clean_url = f"https://api.notion.com/v1/databases/{DATABASE_ID}"
    try:
        res = requests.get(clean_url, headers=NOTION_HEADERS, timeout=15)
        if res.status_code == 200:
            return res.json().get("properties", {})
        else:
            print(f"Schema status: {res.status_code} - {res.text[:100]}")
...
def clean_json_response(raw_text):
    """Gemini output se pure JSON extract karta hai bina markdown formatting error ke"""
    text = raw_text.strip()
    if text.startswith("```"):
...
def safe_send_message(chat_id, text, parse_mode="Markdown"):
    """Telegram message length limit (4096 chars) ko safe chunking se bhejta hai"""
    max_chunk = 3900
...
def build_colored_blocks(markdown_text):
    """
    Atish Mathur standard multi-color keyword styling:
    Format: {{color:keyword}} ya {{keyword}}
    Supported Colors: blue, green, orange, purple, pink, red, yellow, brown
    """
...
def append_blocks_in_chunks(page_id, blocks):
    ...
    clean_url = f"[https://api.notion.com/v1/blocks/](https://api.notion.com/v1/blocks/){page_id}/children"
    for i in range(0, len(blocks), 80):
        batch = blocks[i:i+80]
        try:
            requests.patch(clean_url, headers=NOTION_HEADERS, json={"children": batch}, timeout=20)
        except Exception as e:
            print(f"Block appending error: {e}")
...
def create_notion_entry(art):
    ...
    # 1. Article Title (English - Searchable)
    ...
    # 9. Topic Tags (2 to 5 Specific tags)
    ...
    if tag_col and art.get("topic_tags"):
        ...
        props[tag_col] = {"multi_select": [{"name": str(t)} for t in t_list[:5]]}

    # 10. PYQ Reference (Exact year + Question match)
    ...
    # 11. Quote (Hindi - Dedicated for Essay/GS4)
    ...
    # 12. Ethics Issue-Example (Hindi - Short & Sharp)
    ...
    # 13. Philosophy Issue-Example (Hindi - Short & Sharp)
    ...
    # 14. Mains Question (Hindi - Tough/Application-based)
    ...
    # 15. Mains Answer Framework (Hindi - Intro/Body/Conclusion)
    ...
    clean_pages_url = "[https://api.notion.com/v1/pages](https://api.notion.com/v1/pages)"
    try:
        res = requests.post(clean_pages_url, headers=NOTION_HEADERS, json=payload, timeout=25)
        if res.status_code == 200:
...
def send_welcome(message):
    txt = (
        ...
        "**Standard Rules:**\n"
        "1️⃣ **Stage 1 (Filter):** Newspaper se 4–6 high-yield non-political editorials + 3 PIB releases ki 15-Year PYQ Linked list milegi.\n"
        "2️⃣ **Stage 2 (Upload):** Aapke chunav ke baad AIR-1 standard Hindi analysis (multi-color keywords, 350-450 words, Ethics, Philosophy, Mains Framework) Notion me sync hoga."
    )
...
def handle_pdf_document(message):
    ...
    try:
        ...
        bot.edit_message_text(
            "🧠 Gemini 1.5 Flash se PDF analyze ho raha hai... (Stage 1 Selection List ban rahi hai)",
